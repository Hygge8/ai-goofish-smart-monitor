...
def _score_line(item: dict[str, Any]) -> str:
    ...
    total = item.get("total_score") or score.get("total_score")
    if total in (None, ""):
        return ""
    return (
        f"综合评分：**{total}/100**\n\n"
        f"> 文本 {score.get('text_score', '-')}｜价格 {score.get('price_score', '-')}｜"
        f"卖家 {score.get('seller_score', '-')}｜图片 {score.get('vision_score', '-')}｜"
        f"置信度 {score.get('confidence_score', '-')}"
    )
